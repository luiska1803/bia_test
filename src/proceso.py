...
from config import APP_NAME, CSV_FILE, USE_SPARK, RESULTS_DIR, CHUNK_SIZE, WORKERS
...
def toma_de_coordenadas_UK(df):
    ...
    if isinstance(df, SparkDataFrame):
        df_filtrado = df.filter(
            (col("lat") >= 49.9) & (col("lat") <= 60.9) &
            (col("lon") >= -8.6) & (col("lon") <= 1.8)
        )
    else:
        df_filtrado = df[
            (df['lat'] >= 49.9) & (df['lat'] <= 60.9) &
            (df['lon'] >= -8.6) & (df['lon'] <= 1.8)
        ]
...
def procesar_coordenadas_paralelo(
    ...
    """
    if isinstance(df, SparkDataFrame):
        coords = df.select("lat", "lon").rdd.map(lambda row: (row["lat"], row["lon"])).collect()
    else: 
